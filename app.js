...
class AuditScene extends Phaser.Scene {
    ...
    next() {
        ...
        const targetY = (q.axis === 'y') ? -(last.y + (isM ? 2.5 : 3.4)) * GRID : -(last.y + 0.5) * GRID;
    ...
    spawnGhosts(origin, q) {
        ...
        q.options.forEach((opt, idx) => {
            ...
            const isM = window.innerWidth < 768;
            const ox = (q.axis === 'x') ? (isM ? (idx - 180) : 0) : (isM ? 130 : 200);
            const oy = (q.axis === 'x') ? (isM ? (idx * 200 - 150) : (idx * 120 - 150)) : (isM ? (50 * idx) + 100 : 0);
    ...
    select(val, target) {
        ...
        document.getElementById('card').style.display = 'none';
        // Desvanecer suavemente los elementos antes de destruirlos 
        this.ghosts.forEach(g => {
            g.el.style.opacity = '0';
            g.el.style.transition = 'opacity 0.6s ease-out';
            setTimeout(() => { g.gfx.forEach(v => v.destroy()); g.el.remove(); }, 600);
        });
        this.ghosts = [];
    ...
    finish() {
        ...
        btnReveal.onclick = () => {
            ...
            c.innerHTML = `
                ...
                <div class="desc-scroll" style="margin-top:10px;color:#bbb; white-space: pre-wrap;">${res.desc}</div>
                <hr style="border: 1px solid rgba(130, 23, 206, 0.4); margin: 20px 0;">
                <div class="scores-container" style="text-align: left; font-size: 0.95rem;">
